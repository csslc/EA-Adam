...
parser.add_argument('--config', type=str, default='configs/fusion.yml', help = 'pre-config file for training')
...
parser.add_argument('--is_l1loss', type=float, default= 0.01, help = 'use l1 loss for optimization')
...
if __name__ == '__main__':
    ...
    if args.resume_expert is not None:
        ...
        if len(ckpt_files) != 0:
            ...
            experiment_path = args.resume_expert
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for iter, batch in enumerate(train_dataloader):
            ...
            # convert to [-1,1]
                ...
                loss_p += args.is_dists * DISTS_func(sr_con, hr_con, require_grad=True, batch_average=True)
            if args.is_gan:
                fake_g_pred = model_d(sr)
                loss_p += args.is_gan * (gan_loss(fake_g_pred, True, is_disc=False))
            if args.is_LDL:
        ...
        if epoch % args.test_every == 0:
            ...
            for valid_dataloader in valid_dataloaders:
                ...
                for lr, hr in tqdm(loader, ncols=80):
                    ...
                    lpips = torch.mean(loss_lpips_func((sr/255.)*2-1, (hr/255.)*2-1)).item()
                    avg_psnr += psnr
                    avg_ssim += ssim
                    avg_lpips += lpips
